...
resource "aws_instance" "three-tier-manoj" {
  ...
  tags = {
    BuiltWith = "terraform"
    Name      = "three-tier-manoj"
  }

  vpc_security_group_ids = ["${ aws_security_group.three-tier-manoj-security-group.id }"] # attaching security group
